...
async function main() {
  ...
  const passwordHash = await bcrypt.hash('12345-dev', salt);
  const demoHash = await bcrypt.hash('usuario_demo', salt);
  const arHash = await bcrypt.hash('ar', salt);

  /*
  ////////// Consola generadores /////////////////////
  # Opción 1: Usando node en terminal
  node -e "require('bcrypt').hash('12345-dev'', 10).then(h => console.log(h))"

  # Opción 2: Script rápido
  node -e "require('bcrypt').hash('usuario_demo', 10).then(h => console.log(h))"
  */
