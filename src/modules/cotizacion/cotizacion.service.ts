...
const ESTADOS_COTIZACION_RECHAZABLE = ['cotizacion_nueva', 'opcion_primaria', 'opcion_secundaria']

// Estados de solicitud que permiten recibir cotizaciones
const ESTADOS_SOLICITUD_PARA_COTIZAR = ['en_revision']
...
export class CotizacionService {
  ...
  async crearCotizacion(
    ...
    if (!ESTADOS_SOLICITUD_PARA_COTIZAR.includes(solicitud.estado_solicitud.slug)) {
    ...
    const [estadoCotizacionNueva, estadoCotizacionAnulada, estadoSolicitudCotizacionCargada] =
      await Promise.all([
        this.prisma.estado_cotizacion.findUnique({ where: { slug: 'cotizacion_nueva' } }),
        this.prisma.estado_cotizacion.findUnique({ where: { slug: 'cotizacion_anulada' } }),
        this.prisma.estado_solicitud.findUnique({ where: { slug: 'cotizacion_cargada' } }),
      ])

    if (!estadoCotizacionNueva || !estadoCotizacionAnulada || !estadoSolicitudCotizacionCargada) {
    ...
    if (data.cotizacion_anterior_id !== null && data.cotizacion_anterior_id !== undefined) {
      ...
      if (!cotizacionAnterior) {
        throw new NotFoundException(`Cotización anterior con ID ${data.cotizacion_anterior_id} no encontrada`)
      }

      if (cotizacionAnterior.solicitud_id !== solicitudId) {
        throw new BadRequestException('La cotización anterior no pertenece a esta solicitud')
      }

      // Anular cotización anterior
    ...
    const esReemplazo = data.cotizacion_anterior_id !== null && data.cotizacion_anterior_id !== undefined

    return {
      success: true,
      message: esReemplazo ? 'Cotización reemplazada correctamente' : 'Cotización creada correctamente',
      data: {
        cotizacion: {
          id: cotizacion.id,
  ...
  async rechazarCotizacion(
    ...
    if (!data.comentario || data.comentario.trim() === '') {
      throw new BadRequestException('El comentario es obligatorio para rechazar una cotización')
    }

    const cotizacion = await this.prisma.cotizacion.findUnique({
      where: { id: cotizacionId },
      include: { estado_cotizacion: true, solicitud: { include: { estado_solicitud: true } } }
    })
    ...
    if (!ESTADOS_COTIZACION_RECHAZABLE.includes(cotizacion.estado_cotizacion.slug)) {
      throw new BadRequestException(
    ...
    // Obtener estados necesarios
    const [estadoCotizacionRechazada, estadoSolicitudEnRevision] = await Promise.all([
      this.prisma.estado_cotizacion.findUnique({ where: { slug: 'cotizacion_rechazada' } }),
      this.prisma.estado_solicitud.findUnique({ where: { slug: 'en_revision' } }),
    ])
  ...
  async seleccionarCotizacion(
    ...
    const slugNuevoEstado = data.preferencia === 'OPCION_PRIMARIA' ? 'opcion_primaria' : 'opcion_secundaria'

    const [estadoNuevoCotizacion, estadoSolicitudPendiente] = await Promise.all([
      this.prisma.estado_cotizacion.findUnique({ where: { slug: slugNuevoEstado } }),
      this.prisma.estado_solicitud.findUnique({ where: { slug: 'pendiente' } }),
    ])
  ...
  async reportarNovedad(
    ...
    if (!data.comentario || data.comentario.trim() === '') {
      throw new BadRequestException('El comentario es obligatorio para reportar una novedad')
    }
    ...
    const [estadoNovedadCotizacion, estadoNovedadSolicitud] = await Promise.all([
      this.prisma.estado_cotizacion.findUnique({ where: { slug: 'novedad' } }),
      this.prisma.estado_solicitud.findUnique({ where: { slug: 'novedad' } }),
    ])

    if (!estadoNovedadCotizacion || !estadoNovedadSolicitud) {
      throw new NotFoundException('Estado NOVEDAD no encontrado en la base de datos')
    }
